...
public class RenderHandler 
{
    ...
    //Takes the info from the renderRectangle or renderImage method.
    public void renderArray(int[] renderPixels, int renderWidth, int renderHeight,
                int xPosition, int yPosition, int xZoom, int yZoom, boolean fixed) {//16, 16, 48, 48, 2, 2, true
    ...
    public void setPixel(int pixel, int x, int y, boolean fixed) {
        //-480  -468     -432    -384         0    -468       1000        0     -384       800    
        int pixelIndex = 0;
        if(!fixed) {
            
            if(x >= camera.x && y >= camera.y && x <= camera.x + camera.w && y <= camera.y + camera.h) { //if x & y are in the camera
                //Coordinates of the pixel on the screen that we are attempting to copy the current image pixel onto
                //Offset points by the camera. Adding would offset in the wrong direction. Our camera was moved left and up
                //Therefore our image was rendered right and down
                //                0 - -468      +  0 - -384      * 1000            = 384,468
            ...
            if(x >= 0 && y >= 0 && x <= camera.w && y <= camera.h) {
              pixelIndex = x + y * view.getWidth();  //48,048
            }
